...
void SpecificWorker::compute() {
    ...
    switch (state) {
        ...
        case State::APPROACHING:
            approach_state(objects, current_line);
            break;
        case State::WAITING:
            ...
            //robot.set_current_speed(v[0], v[1]);
            //omnirobot_proxy->setSpeedBase(v[0], v[1], v[2]);
            /// eye tracking: tracks  current selected object or  IOR if none
            eye_track(robot);
            draw_top_camera_optic_ray();
            qInfo()<< robot.get_robot_target_coordinates().x()<< robot.get_robot_target_coordinates().y()<<robot.get_robot_target_coordinates().z()<< robot.has_target();

            // DWA algorithm
            auto [adv, rot, side] = dwa.update(robot.get_robot_target_coordinates(), current_line,
                                               robot.get_current_advance_speed(), robot.get_current_rot_speed(),
                                               viewer);
...
SpecificWorker::get_multi_level_3d_points_omni(const cv::Mat &depth_frame) {
    ...
    for (auto &p: points)
        p.resize(consts.num_angular_bins, Eigen::Vector2f(consts.max_camera_depth_range,
                                                          consts.max_camera_depth_range));   // angular resolution
    ...
    for (int u = 0; u < depth_frame.rows; u++)
        for (int v = 0; v < depth_frame.cols; v++) {
            ...
            for (auto &&[level, step]: iter::range(consts.depth_lines_min_height, consts.depth_lines_max_height,
                                                   consts.depth_lines_step) | iter::enumerate)
...
void SpecificWorker::move_robot(Eigen::Vector2f force) {
    ...
    try {
        ...
        float rot = atan2(force.x(), force.y()) -
                    0.9 * current_servo_angle;  // dumps rotation for small resultant force
        float adv = force.y();
        float side = force.x();
...
void SpecificWorker::search_state(const RoboCompYoloObjects::TObjects &objects) {
    qInfo() << "entra en search state";
    //Eigen::Vector3f v{0, 0, 0.5};
    //le pides el current target, si es -1  significa que es nuevo, hay que asignarle con setcurrenttarget el objeto
    //front coge el primer elemento del array
    ...
    if (auto it = std::find_if(objects.begin(), objects.end(),
        ...
        qInfo() << "set pure rotation";
        qInfo() << robot.get_pure_rotation();
    }
}

void SpecificWorker::approach_state(const RoboCompYoloObjects::TObjects &objects,
                                               const std::vector <Eigen::Vector2f> &line) {
    qInfo() << "entra en approach state";
    if (robot.get_distance_to_target() < 800) {
        state = State::SEARCHING;
    } else {
        if (auto it = std::find_if(objects.begin(), objects.end(),
                                   [r = robot](auto &a) {
            return a.type == r.get_current_target().type; }); it != objects.end()){
            robot.set_current_target(*it);
        }
    }
}
...
void SpecificWorker::draw_forces(const Eigen::Vector2f &force, const Eigen::Vector2f &target,
                                 const Eigen::Vector2f &res) {
    static std::vector<QGraphicsItem *> items;
    ...
    items.push_back(item);
    items.push_back(
            viewer->scene.addLine(viewer->robot_poly()->pos().x(), viewer->robot_poly()->pos().y(), tip2.x(),
                                  tip2.y(), QPen(QColor("blue"), 50)));
    item = viewer->scene.addEllipse(-50, -50, 100, 100, QPen(QColor("blue")), QBrush(QColor("blue")));
    item->setPos(tip1.x(), tip1.y());
    items.push_back(item);
    items.push_back(
            viewer->scene.addLine(viewer->robot_poly()->pos().x(), viewer->robot_poly()->pos().y(), tip3.x(),
                                  tip3.y(), QPen(QColor("green"), 50)));
...
void SpecificWorker::draw_dynamic_threshold(float threshold) {
    ...
    items.push_back(viewer->scene.addEllipse(-threshold, -threshold, 2 * threshold, 2 * threshold,
                                             QPen(QColor("yellow"), 20)));
}
